<?php
	...
	class Parser
	{
		...
		protected function checkTemplate($template)
		{
			...
			$templateBasedir        = strpos($template, DIRECTORY_SEPARATOR) ? dirname(
					$template
			...
			foreach ($templateDirectories as $templateDirectory) {
				...
				$parsedTemplateFilepath = $this->config['cache_dir'] . $templateName . "." . md5(
						$templateDirectory . serialize($this->config['checksum'])
					) . '.rtpl.php';
			...
			if ($this->config['debug'] || !file_exists($parsedTemplateFilepath) || (filemtime(
			...
			if ($codeSplit) //read all parsed code
			{
				foreach ($codeSplit as $html) {
					...
					elseif (preg_match($tagMatch['ternary'], $html, $matches)) {
						$parsedCode .= "<?php echo " . '(' . $this->varReplace(
								$matches[1],
								$loopLevel,
								$escape = true,
								$echo = false
					...
					elseif (preg_match($tagMatch['constant'], $html, $matches)) {
						...
						foreach (static::$registered_tags as $tags => $array) {
							if (preg_match_all('/' . $array['parse'] . '/', $html, $matches)) {
								$found = true;
								$parsedCode .= "<?php echo call_user_func( static::\$registered_tags['$tags']['function'], " . var_export(
										$matches,
										1
									) . " ); ?>";
							}
		...
		protected function varReplace($html, $loopLevel = null, $escape = true, $echo = false)
		{
			...
			if (!empty($loopLevel)) {
				$html = preg_replace(
					array('/(\$key)\b/', '/(\$value)\b/', '/(\$counter)\b/'),
					array('${1}' . $loopLevel, '${1}' . $loopLevel, '${1}' . $loopLevel),
					$html
				);
			}
		...
		protected function blackList($html)
		{
			...
			if (empty($this->config['black_list_preg'])) {
				$this->config['black_list_preg'] = '#[\W\s]*' . implode(
						'[\W\s]*|[\W\s]*',
						static::$black_list
					) . '[\W\s]*#';
			}
