		...
		public function Authenticate($username, $password, $authtype = 'LOGIN', $realm = '', $workstation = '')
		{
			...
			switch ($authtype) {
				...
				case 'NTLM':
					 ...
					 ** PROTOCOL Documentation http://curl.php.haxx.se/rfc/ntlm.html#ntlmSmtpAuthentication
					 */
					...
					if (!$ntlm_client->Initialize($temp)) { //let's test if every function its available
						...
						if ($this->do_debug >= 1) {
							$this->edebug(
								"You need to enable some modules in your php.ini file: " . $this->error["error"] . $this->CRLF
							);
						}
			...
			while (is_resource($this->smtp_conn) && !feof($this->smtp_conn)) {
				...
				if ($endtime) {
					if (time() > $endtime) {
						if ($this->do_debug >= 4) {
							$this->edebug(
								"SMTP -> get_lines(): timelimit reached (" . $this->Timelimit . " seconds) <br />"
							);
						}
						break;
					}
