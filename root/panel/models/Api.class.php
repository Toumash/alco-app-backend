    ...
    public function userUpload($JSON)
    {
        if (handleLogin($this->db, $JSON['login'], $JSON['password'], FALSE) == 'ok') {
            ...
            foreach ($alcohols as $row) {
                ...
                $ok = $this->db->query("INSERT INTO user_alcohols(NAME,PRICE,TYPE,SUBTYPE,VOLUME,PERCENT,DEPOSIT) VALUES ('$name',$price,$type,$subtype,$volume,$percent,$deposit)");
                if ($ok == false) {
                    break;
                }
            }
        ...
        if (!empty($input)) {
            ...
            if ($this->login($login, $input['password']) == R_OK) {
                ...
                if (!empty($input['id']) && !empty($input['content'])) {
                    ...
                    $query_exist = $this->db->query("SELECT EXISTS(SELECT 1 FROM main_alcohols where ID=$id) as exist");
                    ...
                    if ($result['exist'] == 1) {
                        ...
                        if ($insert_result == true) return R_OK;
                        else return R_ERROR;
                        //print_r($data);
...
// Config Values:
            ...
            $status = submitBug($title, $description, $user, $bitBucketAccount, $bitBucketRepo, $basicAuth, null, 'new', $priority, $kind);
            if ($status === FALSE) {
                return R_ERROR;
            } else {
                //echo("<span class='bugformsuccess'>Thank you, your bug <b># ".$status['issueid']."</b> has been submitted.</span>");
                sendBugEmail($user, $status['issueid'], $companyName, $status['issueurl']); // Leave URL parameter blank if you don't want it in the email.
                return R_OK;
            }
        }
    }
        ...
        if (!empty($input)) {
            ...
            if ($this->login($login, $input['password']) == R_OK) {
                ...
                if (!empty($input['id']) && !empty($input['content'])) {
                    ...
                    if ($result['exist'] == 1) {
                        $time = date("Y.m.d H:i:s");
                        $query_string_insert = "INSERT INTO alcohol_comments(alcoholID,userID,content,time) VALUES ($id,{$this->profileID},'{$content}','{$time}')";
                        $insert_result = $this->db->query($query_string_insert);
                        Log::d(mysqli_error($this->db));
                        Log::d($query_string_insert);
                        if ($insert_result == true) return R_OK;
