...
//$mail->AddAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
		...
		public function index(
			...
			if (!empty($newLogin) && !empty($newEmail) && !empty($newPassword)) {
				...
				$register_errors = self::validateRegister(
					$newLogin,
					$newEmail,
					$newPassword,
					$reNewPassword
				...
				if (empty($register_errors)) {
					...
					if (mysqli_num_rows($result_verify_email) == 0) {
						...
						$resultInsert  = $this->db->query(
							"INSERT INTO users(login,email,password,activation) VALUES(' {$REGF['login']}','{$REGF['email']}','{$finalPassword}','{$activation}')"
						);
						if (!$resultInsert) {
							$this->echo_contact_admin();
						...
						if ($this->db->affected_rows == 1) {
							...
							$message .= '<a href="http://dev.code-sharks.pl/manage?v=a&email=' . urlencode(
									$newEmail
								) . '&key=' . $activation . '">Aktywuj</a>';
						...
						<form action=" " method="POST">
							<table>
								...
								<tr>
									...
									<td><input type="email" name="newEmail" id="newEmail"
