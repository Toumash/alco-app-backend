...
class AlcoholDisplay
{
    ...
    public function startModule($action, $id)
    {
        $view = 'db';
                        ...
                        if (isset($_GET['db'])) {
                            switch ($_GET['db']) {
                                ...
                                case 'main':
                                {
                                    ...
                                    $successDelete = $this->db->query($SQL_delete) or die('error' . mysqli_error($this->db));
...
//$tdeposit = $db->real_escape_string($_POST['DEPOSIT']);
            ...
            if ($this->validateUserInput($tname, $ttype, $tsubtype, $tprice,
                $tvolume, $tpercent, $tdeposit)
            ) {
                ...
                if ($successAdd) {
                    ...
                    echo '<br/>';
                    Log::d('DB:' . $database_name . '::' . $_SESSION['user'] . ': ADDED id:' . $this->db->insert_id);
                } else {
    ...
    public function validateUserInput($_name, $_type, $_subtype, $_price,
                                      $_volume, $_percent, $_deposit)
    {
        //TODO: validate iT!!!
        return true;
    }
