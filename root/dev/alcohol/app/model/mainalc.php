<?php
	...
	class MainalcModel extends Model
	{
		...
		{
			$query = $this->pdo->query("SELECT * FROM main_alcohols LIMIT 2000");
			$array = array();
...
//return $this->select('user_alcohols', '*', null, null, 2000);
		...
		public function fetchAllWithTypes()
		{
			$query = $this->pdo->query(
				"SELECT u.ID,u.NAME,u.PRICE,u.VOLUME,u.PERCENT,u.DEPOSIT,t.name as type,s.name as subtype FROM main_alcohols as u,alcohol_types as t,alcohol_subtypes as s WHERE u.TYPE= t.id  AND u.SUBTYPE = s.id  AND u.TYPE = s.typeID  ORDER BY u.NAME ASC LIMIT 2000"
			);
			$array = array();
			while ($row = $query->fetch()) {
				$alc = new Alcohol($row['NAME'], $row['PRICE'], $row['type'], $row['subtype'], $row['VOLUME'], $row['PERCENT'], $row['DEPOSIT']);
