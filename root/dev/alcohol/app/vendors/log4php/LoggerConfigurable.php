	...
	abstract class LoggerConfigurable
	{
		...
		protected function setInteger($property, $value)
		{
			try {
				...
				$value = var_export($value, true);
				$this->warn(
					"Invalid value given for '$property' property: [$value]. Expected an integer. Property not changed."
				);
			}
		}
		...
		protected function setPositiveInteger($property, $value)
		{
			try {
				...
				$value = var_export($value, true);
				$this->warn(
					"Invalid value given for '$property' property: [$value]. Expected a positive integer. Property not changed."
				);
			}
		}
